...
# 翻译函数
def translate_text(text, source_lang, target_lang,model_name,url="http://127.0.0.1:5000/translate"):
    data = {
        'text': text,
    ...
    if response.status_code == 200:
        return response.json()['trans_result']['dst']
    else:
...
def translate_pdf(path, src, tgt,model_name):
    # 翻译文献到新的pdf中
    file_name = path.split("/")[-1]
    new_file_name = 'static/output_file/' + 'translated_' + file_name  # 翻译后的pdf保存路径
    ...
    try:
        for cur_page in cur_pdf:
            ...
            for img in img_list:  # 获取当前页面的图像列表
                ...
                pix_temp.writeImage('static/output_file/' + new_name)
                pix_temp = None  # 释放资源
            ...
            for num in range(len(blks)):  # loop through the blocks
                ...
                if blks[num][-1] == 1:
                    ...
                    try:
                        path_img = 'static/output_file/' + '图片{}.png'.format(imgcount)  # 当前页面第几个图片的位置
                ...
                if num < len(blks) - 1:
                    ...
                    if (abs(blks[num + 1][1] - blks[num][3]) <= 1.0 and abs(
                        ...
                        if reference_flag == 1:
                            ...
                            res = translate_text(trans_pragraph, src, tgt,model_name).replace(' ', '')
                            new_page.insertTextbox(r, res, fontname="song", fontfile=FontFile,
                    ...
                    # 两个块y轴距离远的的时候
                        ...
                        if flag == 1:
                            res = translate_text(content, src, tgt,model_name).replace(' ', '')  # 翻译结果去掉汉字中的空格
                            ...
                            if is_figure(trans_pragraph.replace(' ', '')):  # 将该块的判断是否是图片标注
                                res = translate_text(trans_pragraph, src, tgt,model_name).replace(' ', '')  # 翻译结果去掉汉字中的空格
                                new_page.insertTextbox(r, res, fontname="song", fontfile=FontFile,
                                                       fontsize=7, align=fitz.TEXT_ALIGN_CENTER)
                            # 标记在这里之后的都是参考文献
                            elif is_reference(trans_pragraph.replace(' ', '')):
                                reference_flag = 1
                                new_page.insertTextbox(r, '参考文献', fontname="song", fontfile='FontFile',
                                                       fontsize=fonts, align=text_pos)
                            else:
                                res = translate_text(trans_pragraph, src, tgt,model_name).replace(' ', '')  # 翻译结果去掉汉字中的空格
                                if reference_flag == 1:
...
def translate_docx(path, src, tgt,model_name):
    # 打开输入的 docx 文件
    file_name = path.split("/")[-1]
    new_file_name = 'static/output_file/' + 'translated_' + file_name  # 翻译后的pdf保存路径
    doc = Document(path)
    ...
    for paragraph in doc.paragraphs:
        for run in paragraph.runs:
            ...
            if original_text:
                translated_text = translate_text(original_text, src, tgt,model_name)
...
def translate_txt(path, src, tgt,model_name):
    file_name = path.split("/")[-1]
    new_file_name = 'static/output_file/' + 'translated_' + file_name  # 翻译后的pdf保存路径
